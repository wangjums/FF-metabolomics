...
library(pROC)
library(ROCR)
setwd("C:/Users/wangju/Desktop/subgroupE/subE_opt6")#定文件夹位置
...
for(x in files_list){
  ...
  head(predValues)
  testValues = subset(predValues, dataType == "Test")#Extract the test set result提取验证集的结果
  probValues = extractProb(models_rf,testX = testx, testY = testy)#The prediction result of train and test set训练集和验证集的预测结果和实际结果以及概率

  testProbs = subset(probValues, dataType == "Test")#Extract the result and probability of test set
  head(testValues)
  Pred1 = subset(testValues, model == "rf")
  confusionMatrix(Pred1$pred, Pred1$obs)#Generate the confusion matrix生成验证集的混合矩阵
  #绘制ROC曲线
